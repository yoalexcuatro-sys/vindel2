        ...
        <div className="bg-gray-50 min-h-screen py-12">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
               <div className="animate-pulse flex flex-col md:flex-row gap-8">
                   <div className="flex-1 bg-gray-200 h-[500px] rounded-xl"></div>
    ...
    <div className="bg-gray-50 min-h-screen pb-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                {/* Image Gallery */}
                <div 
                    className="bg-black rounded-xl overflow-hidden shadow-sm relative h-[400px] md:h-[500px] flex items-center justify-center group cursor-pointer"
                    onClick={() => setLightboxOpen(true)}
                >
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={images[currentImageIndex]} alt={product.title} className="max-h-full max-w-full object-contain" />
                    
                    {/* Image counter */}
                    <div className="absolute top-4 left-4 bg-black/60 text-white text-sm px-3 py-1 rounded-lg backdrop-blur-md font-medium">
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl shadow-sm p-6">
                    <Link href={`/user/${product.seller.id}`} className="flex items-center mb-4 group cursor-pointer">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={product.seller.avatar} alt={product.seller.name} className="h-14 w-14 rounded-full object-cover border border-gray-100 mr-4 group-hover:opacity-80 transition-opacity" />
                        <div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative aspect-square bg-gray-100">
                      ...
                      {/* Badge PROM si tiene promoción */}
                      {(item as any).promoted && (
                        <div className="absolute top-2 left-1/2 -translate-x-1/2 bg-[#1e3a5f] text-white text-[10px] font-bold px-2.5 py-1 rounded-b-lg" style={{ clipPath: 'polygon(10% 0, 90% 0, 100% 100%, 0 100%)' }}>
                          PROM
                        </div>
                      )}
                      ...
                      {itemImages.length > 1 && (
                        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1">
                          {itemImages.slice(0, 4).map((_, idx) => (
                            <span 
                              key={idx}
                              className={`h-0.5 rounded-full ${idx === 0 ? 'w-3 bg-gray-800' : 'w-2 bg-gray-400'}`}
                            />
                          ))}
                        </div>
                      )}
                    ...
                    {/* Content */}
                    <div className="p-3">
                      <p className="font-bold text-gray-900 text-sm mb-1">{item.price} €</p>
                      <div className="flex items-center justify-between">
                        <p className="text-xs text-gray-600 truncate flex-1 mr-2">{item.title}</p>
                      ...
                      <div className="mt-2">
                        <span className={`text-[10px] px-2 py-0.5 rounded-full border ${
                          ...
                          {item.condition === 'new' ? 'Nou' : 'Folosit'}
                        </span>
                      </div>
      ...
      {lightboxOpen && (
        ...
        >
          ...
          <div 
            className="bg-black/50 backdrop-blur-sm border-t border-white/10 px-4 py-3"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-white/70 text-sm truncate">{product.seller.name}</p>
              ...
              <button
                onClick={() => setLightboxOpen(false)}
                className="px-6 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white font-semibold rounded-full transition-colors flex-shrink-0"
              >
                Trimite Mesaj
              </button>
            </div>
